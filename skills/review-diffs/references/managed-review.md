...
[non-interactive.md](non-interactive.md) in that mode.
...
engine's native subagent/task tool. The tmux examples below retain the existing full-autonomy execution convention;
use it only within the caller's authorized execution policy. Non-interactive
execution uses the supervisor's explicit policy and reports verification gaps.
...
3. Select the mandatory floor and record optional lens candidates
4. Prepare the floor output files
5. Choose run-scoped tmux names and exact window targets
6. Launch the floor reviewers in parallel
7. Give every reviewer the same contract plus one perspective lens
8. Poll until the floor reviewers finish
9. Decide whether specialist optional lenses are actually required; launch only those
10. Perform a manager pass against the contract and the diff
11. Summarize findings to the user
12. Clean up review windows
...
The three lenses are required in managed mode. Use the entrypoint's focused mode for a small, low-risk review when the caller does not require managed execution. Once a managed run starts, do not omit a failed lens or silently change modes.

### Optional — escalate after the floor only when evidence warrants it (cap 2)
...
Selection rule (the manager does this inline; do not start optional lenses merely because a file type matched):

1. Always include the floor.
2. Before the floor runs, use `docs/review/touched.txt` only to record optional **candidates**. A matching surface is not sufficient reason to launch one.
3. After all floor results are available, launch an optional lens only when at least one of these is true:
   - the implementation contract explicitly requires specialist evidence for that surface;
   - a floor finding needs that specialist to confirm severity or reachability;
   - manager inspection finds a concrete high-risk mechanism that the floor did not meaningfully cover.
4. Do not launch an optional lens to obtain generic reassurance, repeat a floor lens, or review a low-risk surface with no concrete uncertainty.
5. Cap escalated optional lenses at 2. If more qualify, keep the two with the highest potential correctness/security impact and cover the rest in the manager pass.
6. **Log every candidate as launched or skipped, with the concrete reason.** No silent escalation or truncation.

Record the floor and optional candidates at the top of `docs/review/manager.md`; after step 9, update each candidate with its launched/skipped reason.

## 4. Prepare the output files

Create output files for the floor first. Create an optional output file only after the step 9 escalation decision:

- `docs/review/requirements.md` (floor)
- `docs/review/correctness.md` (floor)
- `docs/review/security.md` (floor)
- `docs/review/resilience.md` (optional, only if escalated)
- `docs/review/reuse.md` (optional, only if escalated)
- `docs/review/ui-visual.md` (optional, only if escalated)
...
When an issue runner invokes this workflow, use the issue worktree or
...
Create one dedicated tmux window per currently selected perspective. Start with the three floor windows only. Each runs a coding-agent CLI (`codex` for pure-code lenses by default; `claude` for visual lenses) with the caller's authorized permissions.

Model / effort policy:

- The requirements lens runs on Codex `gpt-5.6-sol` with low effort by default. Escalate genuine requirement ambiguity to the manager or user; do not raise model cost solely because the lens is requirements.
...
- The manager pass (step 10) is run serially by the current session and inherits the caller's model / effort. It has no separate model requirement.

### Engine policy

Reviewers are engine-agnostic (see "Vendor-neutral by design"). Pick the engine per lens:

- **Default pure-code lenses to codex.** This set is `correctness`, `security`, `reuse`, `resilience`, `concurrency`, `api-contract`, and `data-migration`. Launch codex using the permission policy described in "Vendor-neutral by design" above, then inject the same lens prompt via tmux.
- **Visual lenses need image-capable execution.** Default `ui-visual` and `i18n-a11y` to Claude as configured below. A caller may select another engine with the required image/browser capabilities; do not silently substitute an engine after failure. Requirements uses Codex `gpt-5.6-sol` low.
...
Launch the floor windows first. Capture each returned window id with
`-P -F '#{window_id}'` and append it to `docs/review/tmux-targets.env`. The optional
commands below are examples for step 9 only:

```bash
# floor
REQ_WIN=$(tmux new-window -P -F '#{window_id}' -n "${review_tag}-req" -c "$(pwd)" 'codex -m gpt-5.6-sol -c model_reasoning_effort="low" --dangerously-bypass-approvals-and-sandbox')
...
# optional — step 9 only, after the floor-based escalation gate selects them;
# run ui-visual only after the trust-boundary caveat above is satisfied
...
Resolve and record each owned window's exact pane id before sending a prompt.
Use the `tmux-sender` named-buffer procedure with that pane id, then capture the
pane to verify submission. Do not paste to a window target or use a shared buffer.
...
- If a touched widget has no golden, use the screenshot/runtime evidence required by the
  contract. Missing golden coverage is blocking only when that coverage is required;
  do not invent a golden infrastructure task for an otherwise verifiable UI change.
...
## 8. Poll until all floor reviewers finish
...
## 9. Optional lens escalation gate

After the floor outputs are complete, compare their concrete findings and gaps with the
optional candidates recorded in step 3. Launch no more than two optional lenses, and only
when they satisfy the evidence rules in step 3. Run selected optional lenses in parallel,
poll them with the same completion checks as the floor, and record why every candidate was
launched or skipped in `docs/review/manager.md`.

If no candidate qualifies, proceed directly to the manager pass. A surface match alone
must never delay the review.

## 10. Manager pass is mandatory

Apply the entrypoint's finding threshold, verification ownership, and completion
rules. A specialist recommendation does not authorize extra implementation scope.
...
- the selected perspectives (with the engine used per lens) and every launched/skipped optional candidate with its reason
- overall assessment
- remaining blockers
- accepted deferrals
...
- For screenshot-driven UI, did the implementation preserve the visual contract and avoid unauthorized baseline/threshold/selector changes? Are golden failures true regressions or intended changes, and is any contract-required visual evidence missing?
...
Use the [entrypoint's standard review format](../SKILL.md#standard-review-format),
or the caller's explicitly required structured result. Do not duplicate or convert
supervisor result files unless the caller's protocol requires it.
