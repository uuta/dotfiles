...
- Required verification には、実装 agent が worktree 内で作れて、read-only・ネットワークなしのレビュアーが確認できる証跡だけを入れる。PR の CI 結果・所要時間、デプロイ・ストア・外部サービスの状態は Required verification に入れず、「merge 前の確認」として担当（supervisor / owner）とタイミングを書く
- 「PR 本文に記録する」とは書かない。「検証結果として報告する」と書く。PR は実装 agent ではなく、レビュー後の工程が作る
- 認証後のUIを検証する場合、認証済み状態の準備・復旧方法、対象画面への操作、各表示状態を再現するデータを検証契約に含める。環境の優先順位や認証保持の注意点は対象プロジェクトの指示から引き継ぐ。
- 起動成功やログイン画面の表示だけでは、認証後の対象画面の runtime verification を完了扱いにしない。認証が失われた場合は復旧または利用可能な別環境で確認し、ユーザーの操作が必要な場合は必要な操作を具体的に示す。
- 必須の画面確認には対象状態ごとのスクリーンショットと確認結果を対応付ける。未確認の状態は未完了として引き継ぎ、unit / widget test や golden 更新で代替しない。必須条件を変更する場合は明示的な合意を記録する。
